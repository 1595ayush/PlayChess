...
var PgnCtrl =
{
	...
	SetBoard: function ()
	{
		var
			mainDiv = Append("div", document.body, "id=mainDiv;",
				"width:542px;height:368px;margin:0px;margin-left:4px;margin-top:100px;border:solid 2px white;overflow: auto;"),
			pgnDiv = Append("div", mainDiv, "id=pgnDiv;",
				"position:absolute;left:440px;top:109px;width:160px;height:345px;margin:0px;margin-left:4px;border:none;overflow: auto;");
		Append("table", pgnDiv, "id=myTable", "float:right;color:#666;margin-right:2px;width:140px;");

		this.boardDiv = Append("div", mainDiv, "id=board;", 
			"width:352px;height:352px;border:solid 1px white;position:relative;top:7px;left:7px;");
		for (var i = 0; i < 64; i++)
		{
			var
				x = i % 8,
				y = parseInt(i / 8),
				clr = ((x + y) & 1) ? "#f0d9b5" : "#b58863";
			Append("div", this.boardDiv, "id=b" + i,
				"background:" + clr + ";width:44px;height:44px;position:absolute;left:"
				+ x * 44 + "px;top:" + y * 44 + "px;");
		}
	},
	...
	SetButtons: function ()
	{
		var ctrlDiv = Append("div", document.body, "id=ctrlDiv;", 
			"width:542px;height:38px;margin:0px;margin-left:4px;border:solid 2px white;");
		var me = this;
		this.AddButton(ctrlDiv, "Reset", function ()
		{
			if (me.moveId == 0)
				return;
	...
	{
		var 
			dx = (val == "Reset" ? "40px;" : val == "Info" ? "80px;" : "4px;"),
			o = Append("input", div, "type=button;value=" + val, 
				"border:solid 2px white;border-radius:30%;padding:4px;margin:6px;margin-left:" + dx);
		o.onclick = action;
	},
	...
	SetCaptured: function ()
	{
		var div = Append("div", document.body, "id=capturedDiv",
			"margin:0px;margin-left:4px;width:542px;height:50px;border:solid 2px white;overflow:auto;");
		Append("div", div, "id=whitesDiv", "width:542px;height:25px;overflow:auto;");
		Append("div", div, "id=blacksDiv", "width:542px;height:25px;overflow:auto;");
		Append("div", document.body, "id=commentsDiv", "padding:2px;width:542px;height:60px;border:solid 2px white;overflow:auto;display:none");
	},
	...
	SetInfo: function ()
	{
		var
			infoDiv = Append("div", document.body, "id=infoDiv",
				"height:100px;color:#ffdd90;display:" + (this.isHeadersVisible ? "block;" : "none")),
			tbl = Append("table", infoDiv, "id=infoTbl",
				"margin:4px;margin-bottom:0px;width:546px;border-collapse:collapse;");
	},
	...
	ResetInfo: function ()
	{
		var tbl = Get("infoTbl");
		Clear(tbl);
		style = "padding-left:4px;border:solid 2px white;font-size:20px;", tr = null;
		for (var i = 0; i < this.headers.length; i++)
		{
			tr = Append("tr", tbl, null, null);
			Append("td", tr, null, "color:#ffdd90;font-size:12px;border:solid 2px white;padding-left:4px").innerHTML = this.headers[i]["key"];
			Append("td", tr, null, "color:#82e2fe;font-size:12px;border:solid 2px white;padding-left:4px").innerHTML = this.headers[i]["val"];
		}
	},
	...
	ResetPgn: function()
	{
		...
		for (var i = 1; i < this.moves.length; i++)
		{
			...
			{
				tr = Append("tr", tbl, null, null);
				td = Append("td", tr, null, "width:20px;text-align:right;font-weight:bold;color:white");
				AppendText(Math.ceil(i / 2) + ".", td);
			}
			td = Append("td", tr, "id=pgn" + i,
				"padding:2px;text-align:center;cursor:pointer;border:solid 1px #cccccc;font-weight:bold;color:white");
	...
	Highlight: function (pawnToPromote)
	{
		...
		for (var i = 1; i < this.moves.length; i++) // Unmark pgn
			Get("pgn" + i).style.backgroundColor = "";
		...
		if (this.moveId > 0)
		{
			...
			td.style.backgroundColor = "gray"; // Mark pgn
			td.focus();
